...
if __name__ == '__main__':
    ...
    layers = [n_x, 7, 1]
    shallow_parameters = L_layer_model(train_set_x, train_set_y, layers, test_set_x, test_set_y, num_epochs=2000,
                                 learning_rate=0.0075, print_cost=True)

    # Train 5 layer model
    print('\nTrain deeper neural net with 3 hidden layers')
    layers = [n_x, 20, 7, 5, 1]
    deep_parameters = L_layer_model(train_set_x, train_set_y, layers, test_set_x, test_set_y, num_epochs=2000,
                                 learning_rate=0.0075, print_cost=True)
    ...
    plt.figure(10)
    plt.title('Prediction for picture:%d' % np.squeeze(pred))
